...
class Window extends PureComponent {
    ...
    handleChange(event) {
        ...
        if(this.state.value.length !== 0){
            this.setState({textOn : true})
        }else if(this.state.value.length !==0 && this.state.value.length < 6){
            this.setState({
                pass : true,
                nonPass : false,
            })
    ...
    render(){
        const { stationDatas, show, okArray, inputOn, sendTextOn, windowUp, value, textOn, pass, nonPass } = this.state;
            ...
            <div className="window-wrapper" style={{ 
                ...
                <div className="window-content">
                    ...
                    <div
                        ...
                        boxShadow: inputOn ? '4px 6px black' : (windowUp ? (textOn ? '4px 6px blue':'0px 0px #cccccc') : '0px 0px #cccccc'),
                        color: inputOn ? 'black' : (windowUp ? (textOn ? 'blue':'#999') : '#999'),
                        borderColor: inputOn ? 'black' : (windowUp ? (textOn ? 'blue':'#cccccc') : '#cccccc'),
                        transform: inputOn ? 'translate(-2px, -2px)' : (windowUp ? (textOn ? 'translate(-2px, -2px)':'none') : 'none'),
                        cursor: inputOn ? 'pointer' : (windowUp ? (textOn ? 'pointer':'not-allowed') : 'not-allowed'),
                        }}>
                        {inputOn ? '여기까지 할래요': (windowUp ? (textOn ?'다 했다!': '거의 다 왔어요!') : stationName)}
                    </div>
                </div>
            </div>
        )
