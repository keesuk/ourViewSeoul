...
export class Pin extends React.PureComponent {
    ...
    render() {
        ...
        const trianglePin = (i) => {
            return (<text fontSize={'75px'} transform={`rotate(38.2, 10, -50)`} fill={'#087A0D'} key={i}>
            ▲ </text>
        )}
        const circlePin = (i) => {
            return (<text fontSize={'70px'} transform={`rotate(38.2, 20, -35)`} fill={'#E20000'} key={i}>
            ● </text>
        )}
        const rectPin = (i) => {
            return (<text fontSize={'70px'} x={'-20px'} y={'5px'} fill={'#006CC4'} key={i}>
            ■ </text>
        )}
        const ellipsePin = (i) => {
            return (<text fontSize={'100px'} x={'-34px'} y={'10px'} transform={`rotate(38.2, 0, 0)`} fill={'#00bfb3'} key={i}>
            ⬮ </text>
        )}
        const rhombusPin = (i) => {
            return (
            <g transform={`rotate(-15, 0, 0)`}>
                <rect width={'55px'} height={'55px'} x={'-20px'} y={'-45px'} transform={`skewX(-30), skewY(0)`} fill={'#e90087'} key={i}/>
            </g>
        )}
            ...
            <g 
                ...
                transform={
                    this.props.size < pinSize 
                        ?"translate(547.1039 352.4816)"
                        :"translate(547.1039 402.4816)"
                    }
            >
                {this.props.size < pinSize 
                    ?<>
                        <rect width={'80px'} height={'4px'} x={'-40px'} y={'55px'} fill={'#999'}/>
                        <rect width={'4px'} height={'70px'} transform={`rotate(38.2, 0, 0)`} fill={'#555'}/>
                        {pinData.map((pin, i) =>
                            <g transform={`translate(0,${-i*40})`} key={pin.locationName}>
                                {pin.locationTagID === "0" ? circlePin(pin.locationName) 
                                : (pin.locationTagID === "1" ? rhombusPin(pin.locationName)
                                : (pin.locationTagID === "2" ? trianglePin(pin.locationName)
                                : (pin.locationTagID === "3" ? rectPin(pin.locationName)
                                : (pin.locationTagID === "4" ? ellipsePin(pin.locationName)
                                : null))))}  
                            </g>
                        )}
...
export class TagOn extends React.PureComponent {
    render() {
        return(
            <>
                <text fontSize={'75px'} transform={`rotate(38.2, 30, -40)`} fill={'#087A0D'} key={i}>
            ▲ </text>        
            </>
        )
    }
