...
class MapShow extends PureComponent {
    ...
    componentDidUpdate(){
        if(location){
            // const an = location.map(station => this.cor[station.name])
            const ann = location.map(station => this.cor[station.name]).map(station => station.innerHTML.split('(').pop().split(')')[0].split(' '))
            const annParse = ann.map(array => {return [parseInt(array[0]), parseInt(array[1])]})
            
            console.log(annParse)
            // console.log(this.cor['Bulgwang'])
