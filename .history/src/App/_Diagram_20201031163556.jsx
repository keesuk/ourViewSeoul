...
export class Pin extends React.PureComponent {
    ...
    render() {
        ...
        const ellipsePin = (i) => {
            return (<text fontSize={'100px'} x={'-20px'} y={'-10px'} transform={`rotate(38.2, 0, 0)`} fill={'#00bfb3'} key={i}>
            ⬮ </text>
        )}
        const rhombusPin = (i) => {
            return (
            <g transform={`rotate(-15, 0, 0)`}>
                <rect width={'50px'} height={'50px'} x={'-10px'} y={'-40px'} transform={`skewX(-30), skewY(0)`} fill={'#e90087'} key={i}/>
            </g>
        )}
