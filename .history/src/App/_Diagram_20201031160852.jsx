...
export class Pin extends React.PureComponent {
    ...
    render() {
        ...
        const trianglePin = () => {
            return (<text fontSize={'75px'} transform={`rotate(38.2, 30, -40)`} fill={'#087A0D'}>
            ▲ </text>
        )}
        const circlePin = () => {
            return (<text fontSize={'70px'} transform={`rotate(38.2, 20, -35)`} fill={'#E20000'}>
            ● </text>
        )}
        const rectPin = () => {
            return (<text fontSize={'70px'} x={'-8px'} y={'5px'} fill={'#006CC4'}>
            ■ </text>
        )}
        const ellipsePin = () => {
            return (<text fontSize={'100px'} x={'-25px'} y={'0px'} transform={`rotate(38.2, 0, 0)`} fill={'#00bfb3'}>
            ⬮ </text>
        )}
        const rhombusPin = () => {
            return (
            <g transform={`rotate(-15, 0, 0)`}>
                <rect width={'50px'} height={'50px'} x={'-5px'} y={'-40px'} transform={`skewX(-30), skewY(-15)`} fill={'#e90087'}/>
            </g>
        )}
            ...
            >
                ...
                {this.props.size < pinSize 
                    ?<>
                        ...
                        {pinData.map((pin, i) =>
                            <>  
                            {pin.locationTagID === "0" ? trianglePin : rhombusPin}
