...
class Window extends PureComponent {
    ...
    fetchMoreData = () => {
        setTimeout(() => {
          this.setState({
            items: this.state.items.concat(this.state.items.from({ length: 3 }))
          });
        }, 1500);
      };
    ...
    render(){
        const { items, show, okArray, inputOn, sendTextOn, windowUp, value, textOn, nonPass, imageStatus, items } = this.state;
