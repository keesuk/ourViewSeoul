...
class SeoulMap extends PureComponent {
    ...
    handleWhatsOk(whatsOk) {
        this.setState(state => ({ 
            pinData : [...state.pinData, whatsOk[1].filter((ok, i) => !whatsOk[0].some(j => i === j))],
            pinValue : [...state.pinValue, whatsOk[2]]
        }))
    }


    render() { 
        const { wheelValue, imgShow, station, posterOn, pinData } = this.state;
