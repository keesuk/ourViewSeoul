...
class Window extends PureComponent {
    ...
    handleOk(childOk) {
        ...
        if(this.state.okArray.length > 6){
            this.setState(state => ({
                sendTextOn: !state.sendTextOn,
            }))
        }
    }
    ...
    render(){
        const { stationDatas, show, okArray, inputOn, sendTextOn } = this.state;
        console.log(sendTextOn)
