...
const colorList     = ['#E20000','#006CC4','#087A0D','#e90087','#00bfb3']
const shapeItems    = ['●','◼︎','▲','◆','⬮']
...
class Img extends Component {
    ...
    finalCanvas() {
        ...
        context.textAlign = 'center'
        if(ImgProp[2]=== shapeItems[4]){
            context.font = '600 380px Noto Sans KR'
            context.rotate(30*Math.PI / 180)
            context.fillText(ImgProp[2], 320, 150)
        }else if(ImgProp[2]=== shapeItems[2]){
            context.font = '600 320px Noto Sans KR'
            context.fillText(ImgProp[2], 250, 350)
        }else if(ImgProp[2]=== shapeItems[0]){
            context.font = '600 300px Noto Sans KR'
            context.fillText(ImgProp[2], 250, 330)
        }else{
            context.font = '600 390px Noto Sans KR'
            context.fillText(ImgProp[2], 250, 380)
        }
    }
    ...
    sendCount() {
        this.props.onOk(this.props.childOk)
    }
    ...
    render() {
        return (
            ...
            ? <canvas
                ...
                height={height}
                style={{
                    width:'100%', height:'100%', marginBottom:'9px'
                }}
                ref={this.canvasRef} 
            />
            :(this.props.fetching 
            ? this.props.choosePage 
            :<canvas 
                width={width} 
                height={height}
                onClick={() => this.clickPoint()}
                style={{
                    width:'100%', height:'100%', cursor:'pointer', marginBottom:'9px'
                }} 
                ref="canvas" 
            />)}
            </>
