...
class Window extends PureComponent {
    ...
    getLocation = async() => {
        await API.get('/'+ this.props.station).then(({data}) => stationData.push(data.list))
        this.setState({items: stationData.from({ length: 3 })})
    }
    ...
    fetchMoreData = () => {
        setTimeout(() => {
          this.setState({
            items: this.state.items.concat(this.state.stationDatas.from({ length: 3 }))
          });
        }, 1500);
      };
    ...
    render(){
        const { show, okArray, inputOn, sendTextOn, windowUp, value, textOn, nonPass, imageStatus, items } = this.state;
        const stationName = stationData[0] && stationData[0].station
            ...
            <div 
                ...
                <div className="window-content">
                    ...
                    <div className="content-computer-sec">알면 패스!</div>
                        ...
                        >
                        { items[0] && items[0].locationData
                            ? items.map((stationData,i) =>  
                                <Img 
                        ...
                        { sendTextOn 
                            ? <div className="content-form" style={{display : windowUp ? '' : 'none'}}>
                                <div className="content-computer">
                                    나에게 {stationData[0] && stationData[0].station}이란..
                                </div>
                                <form >
                                    <input
                                        ref={this.textInput}
