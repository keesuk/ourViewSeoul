...
class Window extends PureComponent {
    ...
    fetchMoreData = () => {
        const stationDatasList = this.state.stationDatas.list

        setTimeout(() => {
            this.setState(state => ({ 
                arr : [...state.arr, stationDatasList[state.arrNum], stationDatasList[state.arrNum+1], stationDatasList[state.arrNum+2], stationDatasList[state.arrNum+3]],
                arrNum : state.arrNum + 4
            }))
        }, 1500)
    }
    ...
    render(){
        ...
        console.log(stationDatas.list[4])
