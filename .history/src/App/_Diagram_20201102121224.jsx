...
export function infoGraphic(location, corArr){
    

    for(let i= 0, n = location && location.length; i < n; i++){
        console.log(i)

        if(location[i].id === 0){
            return(
                null// circlePin(checkSize(location[i].number), `translate(${corArr[i] && corArr[i][0]}, ${corArr[i] && corArr[i][1]})`, i)
            )
        }else if(location[i].id === 1){
            return(
                rectPin(checkSize(location[i].number), corArr[i] && corArr[i][0], corArr[i] && corArr[i][1], i)
            )
        }else if(location[i].id === 2){
            return(
                null// trianglePin(checkSize(location[i].number), `translate(${corArr[i] && corArr[i][0]}, ${corArr[i] && corArr[i][1]})`, i)
            )
        }else if(location[i].id === 3){
            return(
                rhombusPin(checkSize(location[i].number), corArr[i] && corArr[i][0], corArr[i] && corArr[i][1], i)
            )
        }else if(location[i].id === 4){
            return(
                ellipsePin(checkSize(location[i].number), corArr[i] && corArr[i][0], corArr[i] && corArr[i][1], `translate()`, i)
            )
        }
    }
