...
export function isOdd(num) { return num % 2 === 1; }
...
export function randomProp(obj) {
...
export function infoGraphic(location, corArr, i){
    ...
    function startWithOrnamentType(id, corA, corB, i){ 
        ...
        function checkSize(id){
            ...
            else if(id.number < 10) return id.id === 4 ?'225px': (id.id === 3 ?'320px':'162px')
        } 

        function copyHowMany(id, value){
            const moreFifty = [8, 10, 12, 14, 16, 18, 20]
            const underFifty = [1, 2, 3, 4, 5, 6]

            if(id.number > 50){
                const num = new Array(randomize(moreFifty)).fill(value) 
                return makeShape(id.korName, num)

            }else if(id.number >= 10 && id.number <= 50){
                const num = new Array(randomize(underFifty)).fill(value) 
                return makeShape(id.korName, num)
        ...
        function makeShape(kor, num){
            let length = num.length

            const shapeListUpper = ['stairs', 'cross', 'horizon', 'vertical', 'circle']
            const shapeListUnder = ['cross', 'horizon', 'vertical']
            const shape = randomize(length > 6 
                ? shapeListUpper 
                : shapeListUnder)
            const numToMultiply = length > 6 
                ? 40 
                : 60
            const corList = []
            ...
            if(shape === 'stairs'){
                ...
                for(let i = 0; i < length; i++){
                    if(isOdd(i)) x = x + numToMultiply
                    else y = y + numToMultiply
                    
                    corList.push([x, y])
                }
            }

            return mapping(kor, num, corList)
        }

        function mapping(kor, num, corList){
            ...
            return num.map((v, i) => 
                <g  style={{mixBlendMode: 'multiply'}}
                    onMouseOver={() => changeBackground(kor[i])}
                    transform={`translate(
...
const rotate = keyframes`
    ...
    100% { transform: rotateX(180deg) rotateY(180deg) rotateZ(-180deg) translateX(0); }
`;
...
const DiceDiv = styled.div`
    ...
    .front { 
        transform: rotateY(0deg) translateZ(4.8vw);
    ...
    .back { 
        transform: rotateX(180deg) translateZ(4.8vw);
    ...
    .right { 
        transform: rotateY(90deg) translateZ(4.8vw); 
    ...
    .top { 
        transform: rotateX(90deg) translateZ(4.8vw);
