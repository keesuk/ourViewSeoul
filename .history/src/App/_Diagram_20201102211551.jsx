...
export function infoGraphic(location, corArr, i){
    ...
    function typeCheck(id, corA, corB, i){ 
        ...
        if(id.id === 0){
            return copyMachine(circlePin(checkSize(id), `translate(${corA}, ${corB})`, i))
        }// else if(id.id === 1){
        ...
        //     corA = corA + 100
        //     corB = corB - 530
        //     return ellipsePin(checkSize(id), corA, corB, `rotate(30, 0, 0)`, i)
        // }
    }

    function copyMachine(value){
        const moreFifty = [8, 10, 12, 14, 16, 18, 20]
        const underFifty = [2, 3, 4, 5, 6]

        if(id.number > 50){
            
            for(let i = 0; i < randomize(moreFifty); i++){
                return(
                    <g>
                        {value}
                    </g>
