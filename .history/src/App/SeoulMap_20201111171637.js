...
class SeoulMap extends PureComponent {
    ...
    handlePinCor(cor, id) {
        ...
        let pinCor = [...this.state.pinCor, [corNumZero, corNumOne]]
        let posterPin = [...this.state.posterPin, id]
        let pinDatas = [...this.state.pinDatas]
        let pinValues = [...this.state.pinValues]

        for(let i = 0; i<this.state.posterPin.length; i++){
            if(this.state.posterPin[i] === id) {
                pinCor.splice(i, 1)
                posterPin.splice(i, 1)
                pinDatas.splice(i, 1)
                pinValeus.splice(i, 1)
            }
        }

        this.setState({ pinCor : pinCor, posterPin : posterPin })
