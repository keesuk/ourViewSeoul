...
export function infoGraphic(location, corArr, i){
    ...
    function typeCheck(id, corA, corB, i){ 
        ...
        if(id.id === 0){
            ...
            return ellipsePin(checkSize(id), corA, corB, `rotate(20, 0, 0)`, i)
        }
    }
...
export function circlePin(fontsize, transform, key) {
    return (<text fontSize={fontsize} transform={transform} fill={'#E20000'} key={key} mixBlendMode={'multiply'}>
    ● </text>
)}
export function rectPin(fontsize,x, y, key) {
    return (<text fontSize={fontsize} x={x} y={y} fill={'#006CC4'} key={key} mixBlendMode={'multiply'}>
    ■ </text>
)}
export function trianglePin(fontsize, transform, key) {
    return (<text fontSize={fontsize} transform={transform} fill={'#087A0D'} key={key} mixBlendMode={'multiply'}>
    ▲ </text>
)}
export function rhombusPin(transform, size, x, y, key) {
    return (
    transform === null 
    ?<text fontSize={size} x={x} y={y} fill={'#e90087'} key={key} fontFamily={'Arial, Helvetica, sans-serif'} mixBlendMode={'multiply'}>&#9830;</text>
    :<g transform={transform}>
        <rect width={size} height={size} x={x} y={y} transform={`skewX(-30), skewY(0)`} fill={'#e90087'} key={key} mixBlendMode={'multiply'}/>
    </g>
)}
export function ellipsePin(fontsize, x, y, transform, key) {
    return (<text fontSize={fontsize} x={x} y={y} transform={transform} fill={'#00bfb3'} key={key} mixBlendMode={'multiply'}>
    ⬮ </text>
)}
