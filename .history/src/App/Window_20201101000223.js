...
import { Circle, Triangle, Rhombus, Ellipse, Rectangular, randomize, change } from './_Diagram.jsx';
...
const itemList = [ <Ellipse left={'3px'} right={'3px'} size={'19px'} top={'-6px'} on={0} getDown={1}/>, 
                    <Circle left={'3px'} right={'3px'} size={'15px'} on={0} getDown={1}/>, 
                    <Rectangular left={'3px'} right={'3px'} size={'15px'} on={0} getDown={1}/>, 
                    <Triangle left={'3px'} right={'3px'} size={'16.2px'} top={'-1.5px'} on={0} getDown={1}/>, 
                    <Rhombus left={'3px'} right={'3px'} size={'15px'} on={0} getDown={1}/>
                ]
...
class Window extends Component {
    ...
    fetchMoreData() {
        ...
        this.interval = setInterval(()=>{
            this.setState({choose : this.change(itemList)})
        }, 100)
    ...
    render(){
        const { show, okArray, inputOn, sendTextOn, windowUp, value, textOn, nonPass, arr, downArr, imgLoaded, fetching, choose } = this.state;
                    ...
                    <div className="header-content">
                        ...
                        <div className="tag-wrapper">
                            {arr.map((oneArr, i) =>
                                ...
                                ? <Circle 
                                    left={'3px'} right={'3px'} size={'15px'} key={i} 
                                ...
                                ? <Ellipse  
                                    left={'3px'} right={'3px'} size={'19px'} top={'-6px'} key={i} 
                                    getDown={downArr.filter(value => value ===i)} 
                                    on={okArray.filter(value => value === i)}
                                /> 
                                : (oneArr && oneArr.locationTagID === nature 
                                ? <Triangle 
                                    left={'3px'} right={'3px'} size={'16.2px'} top={'-1.5px'} key={i} 
                                    getDown={downArr.filter(value => value ===i)} 
                                    on={okArray.filter(value => value === i)}
                                />
                                : null
                            )))))}
                            { fetching ? choose : null }
                        </div>
                    </div>
                </header>
