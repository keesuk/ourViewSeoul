...
export function infoGraphic(location, corArr, i){
    ...
    function typeCheck(id, corA, corB, i){ 
        ...
        if(id.id === 0){
            ...
            return rhombusPin(null, checkSize(id.number), corA, corB, null, i)
        }else if(id.id === 2){
...
export function rhombusPin(transform, size, x, y, skew, key) {
    if(transform === null)transform = 'rotate(-50, -200, 200)'
    if(skew === null)skew = 'skewX(-20), skewY(20)'

    return (
    <g transform={transform}>
        <rect width={size} height={size} x={x} y={y} transform={skew} fill={'#e90087'} key={key}/>
    </g>
)}
...
export class Pin extends React.PureComponent {
    ...
    render() {
        ...
        const rhombusInfo = ['rotate(-15, 0, 0)', '52px', '-15px', '-33px', 'skewX(-30), skewY(0)']
            ...
            >
                ...
                {pinData.map((pin, i) =>
                    <g transform={`translate(0,${-i*40})`} key={pin && pin.locationName}>
                        ...
                        : (pin && pin.locationTagID === 3 ? rhombusPin( rhombusInfo[0], rhombusInfo[1], rhombusInfo[2], rhombusInfo[3], rhombusInfo[4], pin && pin.locationName)
