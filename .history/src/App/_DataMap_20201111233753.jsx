...
const location = {
    "Bulgwang":{"0":52,"1":44,"2":66,"3":66,"4":88,"kor":"불광"},
    "Euljiro4saga":{"0":59,"1":77,"2":88,"3":55,"4":88,"kor":"을지로4가"},
    "Noryangjin":{"0":98,"1":76,"2":85,"3":66,"4":86,"kor":"노량진"},
    "Sujiguoffice":{"0":60,"1":52,"2":76,"3":75,"4":51,"kor":"수지구청"},
    "Wolgye":{"0":88,"1":75,"2":75,"3":34,"4":75,"kor":"월계"}
    }
...
class MapShow extends PureComponent {
    ...
    componentDidMount(){
        ...
        setTimeout(() => {
            let zero = randomProp(location)
            console.log(zero)
            let one = randomProp(location)
            let two = randomProp(location)
            let three = randomProp(location)
            let four = randomProp(location)
            console.log( zero['0'], one['1'], two['2'], three['3'], four['4'])
        }, 1000)
    }
                        ...
                        <TransformComponent>
                            <SvgContainer> 
                                ...
                                <g style={{mixBlendMode: 'multiply'}}>
                                    {this.state.dataLocation !== [] ? this.state.dataLocation.map((one, i) => infoGraphic(one, this.state.corArr[i], i)) : null}
                                </g>
                                : null}
                            </Svg>
                            </SvgContainer>
