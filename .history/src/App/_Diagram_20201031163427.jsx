...
export class Pin extends React.PureComponent {
    ...
    render() {
        ...
        const trianglePin = (i) => {
            return (<text fontSize={'75px'} transform={`rotate(38.2, 30, -40)`} fill={'#087A0D'} key={i}>
            ▲ </text>
        )}
        const circlePin = (i) => {
        ...
        const rectPin = (i) => {
            return (<text fontSize={'70px'} x={'-8px'} y={'5px'} fill={'#006CC4'} key={i}>
            ■ </text>
        )}
        const ellipsePin = (i) => {
            return (<text fontSize={'100px'} x={'-10px'} y={'10px'} transform={`rotate(38.2, 0, 0)`} fill={'#00bfb3'} key={i}>
            ⬮ </text>
        )}
        const rhombusPin = (i) => {
            return (
            <g transform={`rotate(-15, 0, 0)`}>
                <rect width={'50px'} height={'50px'} x={'-5px'} y={'-40px'} transform={`skewX(-30), skewY(-15)`} fill={'#e90087'} key={i}/>
            </g>
        )}
            ...
            >
                {this.props.size < pinSize 
                    ?<>
                        ...
                        {pinData.map((pin, i) =>
                            <g transform={`translate(0,${-i*55})`} key={pin.locationName}>
