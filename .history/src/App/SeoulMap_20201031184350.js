...
class SeoulMap extends PureComponent {
    ...
    handlePinCor(cor) {
        console.log(cor)
        const text = cor.split('')
        
        const wordFirst = '('
        const wordLast = ')'
        const numIndexFirst = text.findIndex((text) => text == wordFirst)
        const numIndexLast = text.findIndex((text) => text == wordLast)
        // const nextWord = text[numIndexFirst + 1]
        console.log(cor.split(wordFirst).pop().split(wordLast)[0])
