...
import { pxStringfier, colorChooser, getKeyByValue } from "./_Diagram.jsx";
...
const ExplainDiagram = styled.div`
    ...
    height: 30vw;
`;
const DiagramPurple = styled.div`
    position: absolute;
    left: 10px;
    margin-top:10px;
...
const DiagramGreen = styled.div`
    ...
    color:#087A0D;
`;
const DiagramYellow = styled.div`
    position: absolute;
    left: 10px;
    margin-top:10px;
    font-size: ${props => props.color};
...
class MapShow extends PureComponent {
    ...
    render() {
        ...
        const { purple, blue, red, green, yellow } = this.state

        const purplePx = pxStringfier(purple)
        const bluePx = pxStringfier(blue)
        const redPx = pxStringfier(red)
        const greenPx = pxStringfier(green)
        const yellowPx = pxStringfier(yellow)
        
        console.log(this.state)
        console.log(getKeyByValue(this.state, colorChooser(colorList[0])))
        ...
        return (
                ...
                {stroke ? null :
                    <ExplainDiv>
                        ...
                        <ExplainNumber>
                            {colorList.map(item => <div style={{color: item}}>{getKeyByValue(this.state, colorChooser(item))}%</div>)}
                        </ExplainNumber>
                        <ExplainDiagram>
                            <DiagramPurple color={purplePx}>&#9670;</DiagramPurple>
                            <DiagramBlue color={bluePx}>■</DiagramBlue>
                            <DiagramRed color={redPx}>●</DiagramRed>
                            <DiagramGreen color={greenPx}>▲</DiagramGreen>
                            <DiagramYellow color={yellowPx}>⬮</DiagramYellow>
                        </ExplainDiagram>
                    </ExplainDiv>
                }
                <TransformWrapper
                    ...
                    <React.Fragment>
                        <TransformComponent>
                            <SvgContainer> 
                                ...
                                <g id="IncheonIntlAirportCargoTerminal1" >
