...
import { pxStringfier, colorChooser, randomize, mapDrawer, ellipsePin, circlePin, rectPin, trianglePin, rhombusPin } from "./_Diagram.jsx";
...
const diagramList = ['◆', '■', '●', '▲', '⬮']
const location = [
    {name : 'Bulgwang', number: 60, id: 0},
    {name : 'Sujiguoffice', number: 80, id: 1},
    {name : 'Euljiro4saga', number: 92, id: 2},
    {name : 'Noryangjin', number: 51, id: 3},
    {name : 'Wolgye', number: 64, id: 4},
]
                    ...
                    <React.Fragment>
                        <TransformComponent>
                            <SvgContainer> 
                                ...
                                >
                                {stroke 
                                ? <g transform={pageUp ? 'translate(10, 0) scale(.7)':'translate(0, 0) scale(1)'}>
                                ...
                                <g>
                                    <g style={{mixBlendMode: 'multiply'}}>
                                        {infoGraphic(location, this.state.corArr)}
                                    </g>
                                </g>
                            </Svg>
                            </SvgContainer>
...
function infoGraphic(location, corArr){
    
    function typeCheck(id, corA, corB, i){ 
        
        if(id.id === 0){
            return ellipsePin(checkSize(id.number), corA, corB, `translate()`, i)
        }else if(id.id === 1){
            return rhombusPin(checkSize(id.number), corA, corB, i)
        }else if(id.id === 2){
            return trianglePin(checkSize(id.number), `translate(${corA}, ${corB})`, i)
        }else if(id.id === 3){
            return rectPin(checkSize(id.number), corA, corB, i)
        }else if(id.id === 4){
            return circlePin(checkSize(id.number), `translate(${corA}, ${corB})`, i)
        }
    }
    ...
    for(let i= 0; i<location.length; i++){
        typeCheck(location[i], corArr[i] && corArr[i][0], corArr[i] && corArr[i][1], i)
    }
}