...
class Window extends PureComponent {
    ...
    handleScroll(event) {
        ...
        for(let i = 0; i < downArr.length; i++){
            if(downArr[i] > element.scrollTop){
                this.setState(state => ({getDown : !state.getDown }))
            }
        }
    }
    ...
    render(){
        const { stationDatas, show, okArray, inputOn, sendTextOn, windowUp, value, textOn, nonPass, imageStatus, arr, downArr, getDown } = this.state;
                    ...
                    <div className="header-content">
                        ...
                        <div className="tag-wrapper">
                            {arr.map((oneArr, i) =>
                                ...
                                ? <Circle left={'3px'} right={'3px'} size={'15px'} key={i} getDown={getDown} on={okArray.filter(value => value === i)}/> 
                                : (oneArr && oneArr.locationTagID === shopping
                                ? <Rectangular left={'3px'} right={'3px'} size={'15px'} key={i} getDown={getDown} on={okArray.filter(value => value === i)}/> 
                                : (oneArr && oneArr.locationTagID === museum
                                ? <Rhombus left={'3px'} right={'3px'} size={'15px'} key={i} getDown={getDown} on={okArray.filter(value => value === i)}/> 
                                : (oneArr && oneArr.locationTagID === cafe
                                ? <Ellipse left={'3px'} right={'3px'} size={'19px'} top={'-6px'} key={i} getDown={getDown} on={okArray.filter(value => value === i)}/> 
                                : <Triangle left={'3px'} right={'3px'} size={'16.2px'} top={'-1.5px'} key={i} getDown={getDown} on={okArray.filter(value => value === i)}/>
                            ))))}
                        </div>
                    </div>
                </header>
