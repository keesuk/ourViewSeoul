...
class MapShow extends PureComponent {
    ...
    parsingCor(){
        const parseCor = this.state.locationed.map(station => {
            ...
            if(item[0] > 0 && item[0] < 320) item[0] = item[0] + 100 
            else if(item[0] > 960 && item[0] <= 1280) item[0] = item[0] - 100
            
            if(item[1] > 0 && item[1] < 320) item[1] = item[1] + 100 
            else if(item[1] > 960 && item[1] <= 1280) item[1] = item[1] - 100
                                    ...
                                    ? this.state.do
                                        ? this.state.locationed.map((one, i) => 
                                            <g  
                                                ...
                                                key={i}
                                                onMouseOver={() => console.log(this.state.corArr[i])}
                                            >
                                                {infoGraphic(one, this.state.corArr[i], i)}
                                                <g transform = {`translate(
                                                        ${this.state.corArr[i]&&this.state.corArr[i][0]}, 
                                                        ${this.state.corArr[i]&&this.state.corArr[i][1]} )`}>
                                                    <rect 
                                                        width={100}
                                                        height={100}
                                                        fill={'#fff'}
