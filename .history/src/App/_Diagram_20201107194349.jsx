            ...
            function shapeChoose(num){
                ...
                if(shape === 'cross'){
                    ...
                    if(Number.isInteger(crossOne)){
                        let numToMultiply = 20

                        for(let i = 0; i < num.length; i = i + crossOne){
                            for(let j = 1; j < crossOne + 1; j++){
                                if(crossOne > i){
                                    horList.push([numToMultiply*j,0])
                                }else if(crossOne*2 > i && i >= crossOne){
                                    horList.push([0,numToMultiply*j])
                                }else if(crossOne*3 > i && i >= crossOne*2){
                                    horList.push([-numToMultiply*j,0])
                                }else if(i >= crossOne*3){
                                    horList.push([0, -numToMultiply*j])
                                }
                            }
                        }
                        return (
                            num.map((v, i) => 
                                <g transform={`translate(${horList[i]&&horList[i][0]}, ${horList[i]&&horList[i][1]})`} key={i}>
                                    {v}
                                </g>
                            )
                        )
