...
class Map extends PureComponent {
    ...
    render() {
        const mapList = data.children.map(station => station.children[0].attributes.transform)
        console.log(mapList)
        // const {match, history} = this.props;
        // const mapList = data.map(station => {
        //         <g onClick={()=>history.push(match.path.replace(':station', `${station.attribute.id}`))}>
        //             <text transform={station.children[0].attributes.transform} className={station.children[0].attributes.className}>{station}</text>
        //         </g>
        // })
