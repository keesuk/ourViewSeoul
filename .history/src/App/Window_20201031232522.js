...
class Window extends Component {
    ...
    render(){
        const { show, okArray, inputOn, sendTextOn, windowUp, value, textOn, nonPass, arr, downArr, imgLoaded, fetching } = this.state;
        const stationName = arr[0] && arr[0].station
        const buttonColor = randomize(colorListTag)

        const circleItem = (i, downArr, okArray) => {
            return (
                <Circle left={'3px'} right={'3px'} size={'15px'} key={i} 
                    getDown={downArr.filter(value => value ===i)} 
                    on={okArray.filter(value => value === i)}
                />
            )}
        const rectItem = (i, downArr, okArray) => {
            return (
                <Rectangular left={'3px'} right={'3px'} size={'15px'} key={i} 
                    getDown={downArr.filter(value => value ===i)} 
                    on={okArray.filter(value => value === i)}
                />
        )}
        const rhombusItem = (i, downArr, okArray) => {
            return (
                <Rhombus left={'3px'} right={'3px'} size={'15px'} key={i} 
                    getDown={downArr.filter(value => value ===i)} 
                    on={okArray.filter(value => value === i)}
                /> 
        )}
        const ellipseItem = (i, downArr, okArray) => {
            return (
                <Ellipse left={'3px'} right={'3px'} size={'19px'} top={'-6px'} key={i} 
                    getDown={downArr.filter(value => value ===i)} 
                    on={okArray.filter(value => value === i)}
                /> 
        )}
        const triangleItem = (i, downArr, okArray) => {
            return (
                <Triangle left={'3px'} right={'3px'} size={'16.2px'} top={'-1.5px'} key={i} 
                    getDown={downArr.filter(value => value ===i)} 
                    on={okArray.filter(value => value === i)}
                />
        )}
                ...
                <header className="window-header">
                    <div className="header-content">
                        ...
                        <div className="tag-wrapper">
                            {arr.map((oneArr, i) =>
                                oneArr && oneArr.locationTagID === landmark ? circleItem(i, downArr, okArray)
                                : (oneArr && oneArr.locationTagID === shopping ? rectItem(i, downArr, okArray)
                                : (oneArr && oneArr.locationTagID === museum ? rhombusItem(i, downArr, okArray)
                                : (oneArr && oneArr.locationTagID === cafe ? ellipseItem(i, downArr, okArray)
                                : (oneArr && oneArr.locationTagID === nature ? triangleItem(i, downArr, okArray)
                                : null
                            )))))}
                            { fetching ? setTimeout(()=>{randomize()},200) : null}
                        </div>
                    </div>
                </header>
