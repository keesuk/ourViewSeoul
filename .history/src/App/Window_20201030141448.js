...
class Window extends PureComponent {
    ...
    handleScroll(event) {
        ...
        const downArr = this.state.downArr

        if(scrollBottom === windowBottom){
            this.fetchMoreData()
            this.setState(state => ({downArr: [...state.downArr, element.scrollTop]}))
        }
        for(let i = 0; i < downArr.length; i++){
            if(downArr[i] > element.scrollTop){
                downArr[i] = 0
            }else if(downArr[i] <= element.scrollTop){
                downArr[i] = element.scrollTop
    ...
    render(){
        const { stationDatas, show, okArray, inputOn, sendTextOn, windowUp, value, textOn, nonPass, imageStatus, arr, downArr } = this.state;
                    ...
                    <div className="header-content">
                        ...
                        <div className="tag-wrapper">
                            {arr.map((oneArr, i) =>
                                ...
                                ? <Circle left={'3px'} right={'3px'} size={'15px'} key={i} getDown={downArr.filter(value => value ===i)} on={okArray.filter(value => value === i)}/> 
                                : (oneArr && oneArr.locationTagID === shopping
                                ? <Rectangular left={'3px'} right={'3px'} size={'15px'} key={i} getDown={downArr.filter(value => value ===i)} on={okArray.filter(value => value === i)}/> 
                                : (oneArr && oneArr.locationTagID === museum
                                ? <Rhombus left={'3px'} right={'3px'} size={'15px'} key={i} getDown={downArr.filter(value => value ===i)} on={okArray.filter(value => value === i)}/> 
                                : (oneArr && oneArr.locationTagID === cafe
                                ? <Ellipse left={'3px'} right={'3px'} size={'19px'} top={'-6px'} key={i} getDown={downArr.filter(value => value ===i)} on={okArray.filter(value => value === i)}/> 
                                : <Triangle left={'3px'} right={'3px'} size={'16.2px'} top={'-1.5px'} key={i} getDown={downArr.filter(value => value ===i)} on={okArray.filter(value => value === i)}/>
                            ))))}
                        </div>
                    </div>
                </header>
