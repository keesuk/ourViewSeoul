...
class Window extends PureComponent {
    ...
    fetchMoreData = () => {
        ...
        setTimeout(() => {
            this.setState(state => ({ 
                arr : this.state.arr.concat(stationDatasList[state.arrNum], stationDatasList[state.arrNum+1], stationDatasList[state.arrNum+2], stationDatasList[state.arrNum+3]),
                arrNum : state.arrNum + 4
            }))
        }, 1500)
    }
