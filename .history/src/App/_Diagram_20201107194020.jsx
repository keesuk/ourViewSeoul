...
//, 12, 16, 20
        function copyMachine(id, value){
            ...
            function shapeChoose(num){
                ...
                if(shape === 'cross'){
                    let shapeToDo = num.length/4
                    const horList = []

                    if(Number.isInteger(shapeToDo)){
                        let numToMultiply = 2

                        for(let i = 0; i < num.length; i = i + shapeToDo){
                            for(let j = 1; j < shapeToDo + 1; j++){
                                if(shapeToDo > i){
                                    horList.push([numToMultiply*j,0])
                                }else if(shapeToDo*2 > i && i >= shapeToDo){
                                    horList.push([0,numToMultiply*j])
                                }else if(shapeToDo*3 > i && i >= shapeToDo*2){
                                    horList.push([-numToMultiply*j,0])
                                }else if(i >= shapeToDo*3){
                                    horList.push([0, -numToMultiply*j])
                                }
                            }
                        }
