...
class Window extends Component {
    ...
    render() {
        ...
        if(this.state.locationDatas){
            const { locationDatas } = this.state.locationDatas
            const { imgList } = locationDatas.map(stations => {
                return (
                    <Img key={stations.stationName} points={stations.locationData} className="img-stations"/>
                )}
            )
        }
