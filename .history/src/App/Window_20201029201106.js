...
class Window extends PureComponent {
    ...
    getLocation = async() => {
        ...
        this.setState({ items : data.list.from({ length : 3})})
    }
    ...
    fetchMoreData = () => {
        setTimeout(() => {
          this.setState({
            items: this.state.items.concat(this.state.station.from({ length: 3 }))
          });
        }, 1500);
      };
    ...
    render(){
        const { items, show, okArray, inputOn, sendTextOn, windowUp, value, textOn, nonPass, imageStatus, } = this.state;
        const stationName = items[0] && items[0].station
        console.log(items)
                        ...
                        { sendTextOn 
                            ? <div className="content-form" style={{display : windowUp ? '' : 'none'}}>
                                <div className="content-computer">
                                    나에게 {items[0] && items[0].station}이란..
                                </div>
                                <form >
                                    <input
                                        ref={this.textInput}
