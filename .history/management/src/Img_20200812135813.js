...
class Img extends Component {
    ...
    componentDidMount() {
        ...
        this.context = this.imgRef.current.getContext('2d');

        this.drawImg(svgCanvas, this.context)
    }
    ...
    draw(points) {
        this.context.clearRect(width, 0, width, height);

        points.forEach( point => {
            this.context.beginPath();

            if (points.r) {
                this.context.arc(width + points[0], point[1], point.r, 0, 2 * Math.PI);
                this.context.fill();
            }
        })
    }
    ...
    render() { 
        return <div ref="canvas" ref={this.imgRef}></div> 
    }
}
